...
; v1.1 - Added ToolTip alternative functionality as suggested by BoBo
;
...
;	- Select option to show ToolTips instead from tray menu and tooltips will display instead of parameter insertion
;	- If ToolTips options is chosen, dismiss ToolTip by pressing Esc or Enter
;
...
Menu, Tray, Add, Exit AHK Paramters Script, ExitScript
Menu, Tray, Add, Show ToolTips instead, SelectToolTips

IniRead, ShowTips, Insert AHK Parameters.ini, Main, ShowTips, 0
if ShowTips
	Menu, Tray, Check, Show ToolTips instead
...
+CapsLock::
	...
	Gui, Show, x%A_CaretX% y%A_CaretY% AutoSize
return
...
Enter::
	...
	if ShowTips
	{
		ToolTip, % StrReplace(Trim(PipedParamList, "~"), "~", "`n"), CurrentCaretX, CurrentCaretY + 20
		TipDisplayed := 1
	} else {
...
SelectToolTips:
	ShowTips := !ShowTips
	IniWrite, % ShowTips, Insert AHK Parameters.ini, Main, ShowTips
	Menu, Tray, ToggleCheck, Show ToolTips instead
return
