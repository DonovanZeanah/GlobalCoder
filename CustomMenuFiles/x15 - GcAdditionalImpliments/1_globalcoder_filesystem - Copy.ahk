...
#include 1_globalcoder_parseahk.ahk


g.files[FileExtension].Push(ThisFile) bam done

IsMyMagicTxtFile(Path, Contents) {
    return InStr(Contents, "magic!!!")
}

Rules := [[Func("IsMyMagicTxtFile"), "MagicFileIdentifier"]]

; get a file somehow
for k, v in Rules {
    if (v[1.Call(Path, Contents)) {
        Type := v[2]
        break
    }
}
...
Classes := {"JSON": JSON, "INI": INI, "SINGLE STRING LIST": SingleStringList}
Class := Classes[UserInput]
Loader := new %Class%() ; ignore the horrible syntax, this line functions how you'd think it does

Loader.Parse("whatever")
...
class file {
    ...
    doSomething() {
        switch (this.filetype) {
            case ini:
                ; do something with ini files
            case json:
                ; do something with json files
            case txt:
...
class Gfile extends filesystem {

static this.paths := {}
static this.parsedfiles := {}
static this.types := [filetype_json,filetype_ini,filetype_csv,filetype_kv,filetype_AHK,filetype_CS,filetype_PJCT,Filetype_Hotstrings,filetype_MenuBuilder,filetype_db]

;//CONSTRUCTOR ============
    __new(path_String){
this.paths.push(path_string)
...
parse(){
        for k,v in this.paths
        {
            msgbox, % this.paths.MaxIndex()
            ; put each string into its own var
            sendobjectS := v

            ; ?send that to sub-classes based on file-type ( which is determined by:
            ; first strsplit()?; 
            strsplit(sendobjectS, this.parsedfiles )
           switch (if 
           {
                        ;SplitPath, sendobjectS, OutFileNameS, OutDirS, OutExtensionS, OutNameNoExtS, OutDriveS)
    ...
    ; class filetype_x{ ;//CONSTRUCTOR __NEW{ ;//Methods}
    ; [ Extended Classes ]

class filetype_json{ ;//CONSTRUCTOR __NEW{ ;//Methods ;//END Extension Class}
class filetype_ini{ ;//CONSTRUCTOR __NEW{ ;//Methods ;//END Extension Class}
class filetype_csv{ ;//CONSTRUCTOR __NEW{ ;//Methods ;//END Extension Class}
class filetype_kv{ ;//CONSTRUCTOR __NEW{ ;//Methods ;//END Extension Class}
class filetype_AHK{ ;//CONSTRUCTOR __NEW{ ;//Methods ;//END Extension Class}
class filetype_CS{ ;//CONSTRUCTOR __NEW{ ;//Methods ;//END Extension Class}
class filetype_PJCT{ ;//CONSTRUCTOR __NEW{ ;//Methods ;//END Extension Class}
class Filetype_Hotstrings{ ;//CONSTRUCTOR __NEW{ ;//Methods ;//END Extension Class}
class filetype_MenuBuilder{ ;//CONSTRUCTOR __NEW{ ;//Methods ;//END Extension Class}
class filetype_db
    ;end extensions
} ;END GFILE CLASS
