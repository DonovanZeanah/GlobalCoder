...
{
    _bleb := GuiCtrlObj.Gui["ProgressBar"]
	Gdip_SetProgress(&_bleb, GuiCtrlObj.Value, 0xff0993ea, 0xffbde5ff, GuiCtrlObj.Value "`%")
}
...
Gdip_SetProgress(&Variable, Percentage, Foreground, Background:=0x00000000, Text:="", TextOptions:="x0p y15p s60p Center cff000000 r4 Bold", Font:="Arial")
{
    ...
    Variable.GetPos(&x,&y,&Posw,&Posh)
    ; msgbox x " / " y " / " Posw " / " Posh
	...
	r := Gdip_FillRoundedRectangle(G, pBrushFront, 4, 4, (Posw-8)*(Percentage/100), Posh-8, (Percentage >= 3) ? 3 : Percentage)
	; msgbox "r: " r " / " A_LastError " / G: " G " / pBrushFront: " pBrushFront " / pBitmap: " pBitmap "`r`n"
         ; . "PosW: " PosW " / PosH: " PosH " / hwnd: " Variable.hwnd
	...
	r := SetImage(hwnd, hBitmap)
    ; Variable.Value := "HBITMAP:" hBitmap
    
    Variable.GetPos(&x,&y,&Posw,&Posh)
    ; msgbox Posw " / " Posh " / r: " r " / " A_LastError
    Variable.Move(,,400,50)
