...
if (overlay.BeginDraw()) {
	...
	;draw background
	overlay.FillRoundedRectangle(0,0,overlay.width/2,overlay.height,55,55,(opacity<<24))
	overlay.DrawRoundedRectangle(0,0,overlay.width/2,overlay.height,55,55,0xFFFFFFFF,2)
...
;globalcompletioning globalcompletioning globalcompletioning globalcompletioning
...
a::
string := "thiss is an example"
stepsText.push(string)

up::

inputbox, outvar ,"input",,,,,,,, text
string .= "`n" . outvar
msgbox, % string
stepsText.push("`n" . string)
return
...
Random(min,max) {
	...
	/*if (step = 1) {
		...
		overlay.DrawText("Font: Arial, Size: 18",width*0.005,height*0.3,18,0xFFFFFFFF,"Arial")
		...
		if (objs.length() < 500)
			loop 1
				objs.push(new rain(-500,overlay.width+500,-100,0,(random(200,255)<<24)+(random(95,230)<<16)+(random(200,250)<<8)+random(222,255),overlay.height*2))
		for k,v in objs
