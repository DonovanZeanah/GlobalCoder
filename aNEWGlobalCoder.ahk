...
global MyProgress  := 0
global TotalWords  := 0
global settingsINI := "settings/settings.ini"
global ignoreFiles := ""
;globaql icon := 
...
global FileCount := getItemCount() ; Get amount of items in folder and prepare the menu
PrepareMenu(A_ScriptDir "\CustomMenuFiles") 
RunOtherScripts(A_ScriptDir "\IncludeOtherScripts") ;Run other scripts in the "IncludeOtherScripts" folder
...
; Intro from GDIP library introduction. https://github.com/tariqporter/Gdip/blob/master/Gdip.ahk
...
pBitmap := Gdip_CreateBitmapFromFile("lib\ico\globe.png")
...
PrepareMenu(PATH)
{
	...
	Menu, %PATH%, Icon,% ScriptName " v " Version, %A_ScriptDir%\Lib\Files\Images\ico\campfire.ico 		; Logo
